...
@everywhere begin
    ...
    sourcefile = ["FEM_1_eigen.tcl", "wind-speed.dat", "wind-load-abs.dat", "wind-load-node2.dat", "wind-load-node1.dat"]
    #change to FEM_1.tcl if no frequency analysis is wanted
    ...
    wl_node1_model = Model(df -> [
        [calculate_wind_force(                      
            0, v, 53.0, 50.0, 4, 5.02, 21.10, 3.02, 2.0, 0.0)
        for v in wl_vec]
        for wl_vec in df.wl_base],
        :wl_node1)
    ...
    ext = ExternalModel(
        sourcedir, sourcefile, [max_abs_disp, disp, disp_node2, disp_node3, sim_time], opensees; workdir=workdir, formats=numberformats
    )
