...
@everywhere begin
    ...
    wl_node3_model = Model(df -> [
        [calculate_wind_force(0, v, 53.0, 45.0, 4, 1.25, 5.285, 0.68, 0.57, 0.0)
        for v in wl_vec]
        for wl_vec in df.wl_base
        ], :wl_node3)
    ...
    wl_node8_model = Model(df -> [
        [calculate_wind_force(0, v, 53.0, 35.10, 4, 1.13, 5,79, 1.13, 0.0, 0.0)
        for v in wl_vec]
        for wl_vec in df.wl_base
        ], :wl_node8)
...
capacity = 1.0 #maximum allowed displacements in m
