...
# ---------------- paths ----------------
results_dir = raw"C:\IP\results\SuS\Model_1\plots"
jld_path    = raw"C:\IP\results\SuS\Model_1\20250827_184701_model_1_samples.jld2"
meta_path   = raw"C:\IP\results\SuS\Model_1\20250827_184701metadata (1).json"
...
# helper to fetch a key from JSON3.Object, Dict, etc.
getmeta(m, key) = try getproperty(m, key) catch; try m[key] catch; try m[String(key)] catch; nothing end end end
takefirst(args...) = (for a in args; a !== nothing && return a; end; nothing)

if plotpsdq
    # 1) try to read from metadata JSON
    fdisc     = meta === nothing ? nothing : takefirst(getmeta(meta, :fdisc), getmeta(meta, "fdisc"))
    psdvalues = meta === nothing ? nothing : takefirst(getmeta(meta, :psdvalues), getmeta(meta, "psdvalues"))

    # 2) fallback: rebuild PSD if missing
    if fdisc === nothing || psdvalues === nothing
        @warn "PSD arrays not found in metadata; rebuilding with default Von Kármán params."
        fdisc = collect(0.01:0.01:50.0)
        σ_u, L_u, v_mean = 1.0, 50.8, 30.0
        karman(f) = σ_u^2 * (4L_u / v_mean) / (1 + (2π * f * L_u / v_mean)^2)^(5/6)
        psdvalues = karman.(fdisc)
    end

    # 3) basic checks + cleaning for log axes
    @assert length(fdisc) == length(psdvalues) "fdisc and psdvalues must have the same length."
    mask = (fdisc .> 0) .& isfinite.(psdvalues)
    fplot = fdisc[mask]; pplot = psdvalues[mask]
    if isempty(fplot)
        error("All PSD points were filtered out (nonpositive frequencies or nonfinite PSD). Check inputs.")
    end

    # 4) plot + save
    p_psd = plot(fplot, pplot;
    ...
    savefig(p_psd, joinpath(results_dir, "psd_von_karman.png"))
end
...
# compute the critical one for reporting
crit_val, crit_idx = findmax(max_abs_disp)

# when printing summaries, use crit_idx-based aligned series
t_crit  = collect(times[crit_idx])
u_crit  = collect(disp[crit_idx])
v_full_c   = collect(wl_base[crit_idx])
wl2_full_c = collect(wl_node2[crit_idx])
wl1_full_c = collect(wl_node1[crit_idx])
wl2_full_c = eltype(wl2_full_c) <: Number ? wl2_full_c : to_float_vec(wl2_full_c)
wl1_full_c = eltype(wl1_full_c) <: Number ? wl1_full_c : to_float_vec(wl1_full_c)
v_c   = collect(align_to_window(v_full_c,  length(t_crit)))
wl2_c = collect(align_to_window(wl2_full_c, length(t_crit)))
wl1_c = collect(align_to_window(wl1_full_c, length(t_crit)))

println(">> Reporting maxima for critical sample = $crit_idx (max_abs_disp=$(crit_val) m)")
println("Young's modulus (critical): $(E_vals[crit_idx]) Pa")
println("Maximum wind speed (critical): $(maximum(abs.(v_c))) m/s")
println("Maximum wind load at node 2 (critical): $(maximum(abs.(wl2_c))) kN")
